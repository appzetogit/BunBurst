...
export const updateMenu = asyncHandler(async (req, res) => {
  ...
  const existingMenu = await Menu.findOne({ cafe: cafeId });
    ...
    return {
      ...
      items: Array.isArray(section.items) ? section.items.map(item => {
        ...
        return {
          ...
          // Cafe should NOT be able to overwrite these fields
  ...
  const savedMenu = await Menu.findOne({ cafe: cafeId }).lean();
...
// Get menu by cafe ID (public - for user module)
export const getMenuByCafeId = async (req, res) => {
  try {
    ...
    console.log('[USER MENU] Processing menu for cafe:', cafe._id);
...
// Get add-ons for cafe (filtered by approval status)
export const getAddons = asyncHandler(async (req, res) => {
  ...
  if (!includePending) {
    // For cafe view, show all add-ons (including pending for their reference)
    // For public view, only show approved add-ons
    addons = addons; // Show all for cafe admin
  } else {
...
// Get addons by cafe ID (public - for user module)
export const getAddonsByCafeId = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    // Find cafe by ID, slug, or cafeId - don't filter by isActive
    ...
    if (!cafe) {
      console.log(`[ADDONS] Cafe not found for ID: ${id}`);
      return errorResponse(res, 404, 'Cafe not found');
    }

    console.log(`[ADDONS] Cafe found: ${cafe._id}, name: ${cafe.name}, isActive: ${cafe.isActive}`);

    console.log(`[ADDONS] Cafe found: ${cafe._id}, name: ${cafe.name}`);
    ...
    if (!menu) {
      console.log(`[ADDONS] No menu found for cafe: ${cafe._id}`);
      return successResponse(res, 200, 'No add-ons found', {
        addons: [],
      });
    }

    console.log(`[ADDONS] Menu found for cafe: ${cafe._id}`);
