...
/**
 * Get cafe finance/payout data
 * GET /api/cafe/finance
 * Query params: startDate, endDate (for past cycles)
 */
export const getCafeFinance = asyncHandler(async (req, res) => {
  try {
    const cafe = req.cafe;
    const { startDate, endDate } = req.query;

    // Get cafe ID
    const cafeId = cafe._id?.toString() || cafe.cafeId || cafe.id;
    ...
    // Query for cafe orders - handle multiple cafeId formats
    ...
    console.log(`📊 Finance API - Current cycle orders found: ${currentCycleOrders.length} for cafe ${cafeId}`);
    ...
    return successResponse(res, 200, 'Finance data retrieved successfully', {
      ...
      cafe: {
        ...
        address: cafe.location?.address || cafe.location?.formattedAddress || ''
      }
    });
  } catch (error) {
