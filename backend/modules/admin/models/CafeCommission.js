...
const cafeCommissionSchema = new mongoose.Schema(
  {
    cafe: {
...
// Indexes
cafeCommissionSchema.index({ cafe: 1 });
cafeCommissionSchema.index({ cafeId: 1 });
cafeCommissionSchema.index({ status: 1 });
cafeCommissionSchema.index({ createdAt: -1 });
cafeCommissionSchema.index({ createdBy: 1 });

// Method to calculate commission for an order amount
cafeCommissionSchema.methods.calculateCommission = function(orderAmount) {
  if (!this.status) {
    return {
      commission: 0,
      type: 'amount',
      value: 0,
      rule: null,
      message: 'Commission is disabled for this cafe'
    };
  }
...
// Static method to get commission for a cafe
cafeCommissionSchema.statics.getCommissionForCafe = async function(cafeId) {
  const commission = await this.findOne({ 
    cafe: cafeId,
    status: true 
  }).populate('cafe', 'name cafeId isActive');
  
  return commission;
};

// Static method to calculate commission for a cafe and order amount
cafeCommissionSchema.statics.calculateCommissionForOrder = async function(cafeId, orderAmount) {
  const commission = await this.findOne({ 
    cafe: cafeId,
    status: true 
  });
...
const CafeCommission = mongoose.model('CafeCommission', cafeCommissionSchema);

export default CafeCommission;
