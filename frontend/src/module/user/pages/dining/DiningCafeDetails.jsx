...
export default function DiningCafeDetails() {
        ...
        const fetchCafe = async () => {
            ...
            try {
                ...
                if (response.data && response.data.success) {
                    ...
                    // Fallback: search by name if slug lookup fails directly (though getCafeById usually handles slugs)
                    // For now, assuming direct slug work or we might need the search logic from CafeDetails.jsx
                    setCafe(null)
    ...
    // Helper values
    const coverImage = cafe.coverImage || cafe.profileImage?.url || cafe.logo || "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800&h=600&fit=crop"
    const formattedDistance = "2.4 km away" // Placeholder or calc
    const rating = cafe.rating || cafe.avgRating || 4.5
    const isOpen = cafe.isAcceptingOrders !== false // simplified check
            ...
            <div className="relative h-[45vh] w-full">
                ...
                <div className="absolute bottom-0 left-0 w-full p-5 text-white">
                    ...
                    <p className="text-sm text-gray-300 line-clamp-2 max-w-[90%] mb-2">
                        {cafe.location?.addressLine1 || cafe.address || "Location not available"}
                    </p>
                    ...
                    <div className="flex items-center gap-3 text-sm font-medium mb-3 text-white/90">
                        ...
                        {/* Cost For Two */}
                        <span>{cafe.costForTwo ? `₹${cafe.costForTwo} for two` : "₹1400 for two"}</span>
                    </div>
