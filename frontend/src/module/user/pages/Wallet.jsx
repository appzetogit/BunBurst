...
export default function Wallet() {
  ...
  const getTransactionIcon = (type) => {
    switch (type) {
      case 'addition':
        return <ArrowDownCircle className="h-6 w-6 md:h-7 md:w-7 lg:h-8 lg:w-8 text-highlight" />
      case 'deduction':
        return <ArrowUpCircle className="h-6 w-6 md:h-7 md:w-7 lg:h-8 lg:w-8 text-primary" />
      case 'refund':
        return <RefreshCw className="h-6 w-6 md:h-7 md:w-7 lg:h-8 lg:w-8 text-highlight" />
      default:
        return null
    }
  }
      ...
      <div className="bg-card sticky top-0 z-10 border-b border-border">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center gap-3 px-4 sm:px-6 md:px-8 lg:px-10 py-4 md:py-5">
            <button
              onClick={() => navigate(-1)}
              className="w-9 h-9 md:w-10 md:h-10 flex items-center justify-center hover:bg-muted rounded-full transition-colors flex-shrink-0"
            >
              <ArrowLeft className="h-5 w-5 md:h-6 md:w-6 text-foreground" />
            </button>
            <h1 className="text-lg md:text-xl lg:text-2xl font-bold text-foreground">Wallet</h1>
          </div>
        </div>
      </div>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 md:gap-8 lg:gap-10">
              ...
              <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6 lg:gap-8 flex-1">
                ...
                <div className="relative flex-shrink-0">
                  <div className="w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 xl:w-32 xl:h-32 bg-primary rounded-xl md:rounded-2xl flex items-center justify-center shadow-lg transform rotate-[-5deg]">
                    <div className="w-16 h-16 md:w-20 md:h-20 lg:w-24 lg:h-24 xl:w-28 xl:h-28 bg-white/10 rounded-lg md:rounded-xl flex items-center justify-center backdrop-blur-sm border border-white/20">
                      <IndianRupee className="h-10 w-10 md:h-12 md:w-12 lg:h-14 lg:w-14 xl:h-16 xl:w-16 text-primary-foreground" strokeWidth={2.5} />
                    </div>
                  </div>
                  {/* 3D effect shadow */}
                  <div className="absolute inset-0 bg-primary/20 rounded-xl md:rounded-2xl transform rotate-[-5deg] translate-y-1 -z-10"></div>
                </div>

                {/* Wallet Details */}
                <div className="flex flex-col md:items-start items-center text-center md:text-left">
                  <h2 className="text-xl md:text-2xl lg:text-3xl xl:text-4xl font-bold text-foreground mb-2 md:mb-3">{companyName} Money</h2>

                  {/* Current Balance */}
                  <div className="mb-2 md:mb-3">
                    <p className="text-muted-foreground text-xs md:text-sm lg:text-base mb-1">Current Balance</p>
                    <p className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl 2xl:text-6xl font-bold text-foreground">{formatAmount(currentBalance)}</p>
                  </div>
              ...
              <div className="flex-shrink-0 w-full md:w-auto">
                <Button
                  className="w-full md:w-auto md:min-w-[200px] lg:min-w-[240px] h-12 md:h-14 lg:h-16 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold text-sm md:text-base lg:text-lg rounded-xl transition-all duration-200 flex items-center justify-center gap-2 shadow-md hover:shadow-lg"
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 md:px-5 lg:px-6 py-2 md:py-2.5 lg:py-3 rounded-lg md:rounded-xl text-xs md:text-sm lg:text-base font-medium whitespace-nowrap flex-shrink-0 transition-all ${isSelected
                          ...
                          : 'bg-card border border-border text-foreground/80 hover:bg-muted transition-colors'
                          }`}
                      >
                        {filter.label}
                      </button>
              ...
              {filteredTransactions.length > 0 ? (
                <div className="space-y-3 md:space-y-4">
                  {filteredTransactions.map((transaction) => (
                    <Card key={transaction.id} className="py-0 border-border bg-card hover:shadow-md transition-all duration-200 cursor-pointer">
