  ...
  const handleSocialSignIn = async (provider, providerName = "Social") => {
    ...
    try {
      ...
      try {
        ...
        console.warn("⚠️ Popup sign-in returned no user; falling back to redirect.")
      } catch (popupError) {
        const popupCode = popupError?.code || ""
        const popupMessage = popupError?.message || ""
        console.warn("⚠️ Popup sign-in failed; may fall back to redirect.", { popupCode, popupMessage })
          ...
          <div className="p-6 sm:p-7 space-y-4">
            ...
            <div className="flex items-center justify-center gap-4">
              <button
                ...
                className="h-12 w-12 rounded-full flex items-center justify-center transition-all hover:scale-[1.04] active:scale-[0.98]"
                style={{ background: "#ffffff", border: "1px solid #e5e7eb", boxShadow: "0 4px 12px rgba(0,0,0,0.12)" }}
              ...
              <button
                ...
                className="h-12 w-12 rounded-full flex items-center justify-center transition-all hover:scale-[1.04] active:scale-[0.98]"
                style={{ background: "#1877F2", boxShadow: "0 6px 16px rgba(24,119,242,0.35)" }}
