...
export default function CafeNavbar({
  ...
  // Get cafe name (use prop if provided, otherwise use fetched data)
  const cafeName = propCafeName || cafeData?.name || "Cafe"

  const [location, setLocation] = useState("")

  // Update location when cafeData or propLocation changes
  useEffect(() => {
    let newLocation = ""
    ...
    // Priority 2: Check cafeData location
    else if (cafeData) {
      if (cafeData.location) {
        // Use stored formattedAddress first (from database)
        if (cafeData.location.formattedAddress && 
            cafeData.location.formattedAddress.trim() !== "" && 
            cafeData.location.formattedAddress !== "Select location") {
          // Check if it's just coordinates (latitude, longitude format)
          const isCoordinates = /^-?\d+\.\d+,\s*-?\d+\.\d+$/.test(cafeData.location.formattedAddress.trim())
          if (!isCoordinates) {
            newLocation = cafeData.location.formattedAddress.trim()
            }
        }
        
        // If formattedAddress is not available or is coordinates, try formatAddress function
        if (!newLocation) {
          const formatted = formatAddress(cafeData.location)
        ...
        if (!newLocation && cafeData.location.address && cafeData.location.address.trim() !== "") {
          newLocation = cafeData.location.address.trim()
          }
      }
      
      // Priority 3: Fallback - check if address is directly on cafeData (not in location object)
      if (!newLocation && cafeData.address && cafeData.address.trim() !== "") {
        newLocation = cafeData.address.trim()
        }
    }
  ...
  useEffect(() => {
    ...
    // Listen for status changes from CafeStatus page
    const handleStatusChange = (event) => {
      const isOnline = event.detail?.isOnline ?? false
      setStatus(isOnline ? "Online" : "Offline")
    }

    window.addEventListener('cafeStatusChanged', handleStatusChange)
    
    // Also check localStorage periodically to catch direct changes
    const interval = setInterval(updateStatus, 1000)
    
    return () => {
      window.removeEventListener('cafeStatusChanged', handleStatusChange)
      clearInterval(interval)
    }
  }, [])

  const handleStatusClick = () => {
    navigate("/cafe/status")
  }
