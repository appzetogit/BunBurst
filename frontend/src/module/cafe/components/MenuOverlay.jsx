...
export default function MenuOverlay({ showMenu, setShowMenu }) {
  ...
  const [isAuthenticated, setIsAuthenticated] = useState(() => {
    return localStorage.getItem("cafe_authenticated") === "true"
  })

  // Listen for authentication state changes
  useEffect(() => {
    const checkAuth = () => {
      setIsAuthenticated(localStorage.getItem("cafe_authenticated") === "true")
    }
    ...
    window.addEventListener('cafeAuthChanged', checkAuth)

    return () => {
      window.removeEventListener('storage', checkAuth)
      window.removeEventListener('cafeAuthChanged', checkAuth)
    }
  }, [])
  ...
  const getMenuOptions = () => {
    const baseOptions = [
      { id: 4, name: "All Food", icon: Utensils, route: "/cafe/food/all" },
      { id: 6, name: "Cafe Config", icon: Settings, route: "/cafe/config" },
      { id: 7, name: "Advertisements", icon: Monitor, route: "/cafe/advertisements" },
      { id: 9, name: "Categories", icon: Grid3x3, route: "/cafe/categories" },
      { id: 10, name: "Coupon", icon: Tag, route: "/cafe/coupon" },
      { id: 11, name: "My Business Plan", icon: FileText, route: "/cafe/business-plan" },
      { id: 12, name: "Reviews", icon: MessageSquare, route: "/cafe/reviews" },
      { id: 14, name: "Wallet Method", icon: Settings, route: "/cafe/wallet" },
      { id: 16, name: "Settings", icon: Settings, route: "/cafe/settings" },
      { id: 17, name: "Conversation", icon: MessageCircle, route: "/cafe/conversation" },
      { id: 18, name: "Privacy Policy", icon: Shield, route: "/cafe/privacy" },
      { id: 19, name: "Terms & Condition", icon: CheckSquare, route: "/cafe/terms" },
    ]
    ...
    if (isAuthenticated) {
      ...
      return [
        { id: 1, name: "Login", icon: LogIn, route: "/cafe/login" },
        ...baseOptions
      ]
    }
  }
