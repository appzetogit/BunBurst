...
/**
 * Hook for cafe to receive real-time order notifications with sound
 * @returns {object} - { newOrder, playSound, isConnected }
 */
export const useCafeNotifications = () => {
    ...
    const fetchCafeId = async () => {
      try {
        ...
        if (response.data?.success && response.data.data?.cafe) {
          ...
          const id = cafe._id?.toString() || cafe.cafeId;
          setCafeId(id);
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const socketUrl = `${backendUrl}/cafe`;
    ...
    // Initialize socket connection to cafe namespace
    ...
    socketRef.current = io(socketUrl, {
      ...
      autoConnect: true,
      auth: {
        token: localStorage.getItem('cafe_accessToken') || localStorage.getItem('accessToken')
      }
    });
    ...
    socketRef.current.on('connect', () => {
      ...
      // Join cafe room immediately after connection with retry
      if (cafeId) {
        const joinRoom = () => {
        ...
        console.warn('⚠️ Cannot join cafe room: cafeId is missing');
      }
    });

    // Listen for room join confirmation
    socketRef.current.on('cafe-room-joined', (data) => {
    ...
    socketRef.current.on('reconnect', (attemptNumber) => {
      ...
      // Rejoin cafe room after reconnection
      if (cafeId) {
        socketRef.current.emit('join-cafe', cafeId);
      }
    });
