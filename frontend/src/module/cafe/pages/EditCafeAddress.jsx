...
export default function EditCafeAddress() {
    ...
    const fetchCafeData = async () => {
      try {
        ...
        try {
          ...
          // Try to get cafe name from localStorage, but prefer empty string over hardcoded value
          const savedName = localStorage.getItem("cafe_name") || 
                           localStorage.getItem("cafeName") ||
                           ""
          setCafeName(savedName)
        } catch (e) {
    ...
    try {
      ...
      if (selectedOption === "update_address") {
        ...
        try {
          ...
          const response = await cafeAPI.updateProfile({ location: updatedLocation })
      ...
      <div className="bg-white border-b border-gray-200 px-4 py-3 sticky top-0 z-50 flex items-center gap-3 shrink-0">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1">
            <h1 className="text-base font-bold text-gray-900 truncate">{cafeName}</h1>
