...
const CUISINES_STORAGE_KEY = "cafe_cuisines"
...
export default function OutletInfo() {
  ...
  const [cafeMongoId, setCafeMongoId] = useState("")
  ...
  const handleProfileImageReplace = async (event) => {
    ...
    try {
      ...
      const uploadResponse = await cafeAPI.uploadProfileImage(file)
      ...
      if (uploadedImage) {
        ...
        // Refresh cafe data to get latest from backend
        const response = await cafeAPI.getCurrentCafe()
        const data = response?.data?.data?.cafe || response?.data?.cafe
        if (data) {
          setCafeData(data)
  ...
  const handleCoverImageAdd = async (event) => {
    ...
    try {
      ...
      const currentResponse = await cafeAPI.getCurrentCafe()
      const currentData = currentResponse?.data?.data?.cafe || currentResponse?.data?.cafe
      ...
      for (let i = 0; i < files.length; i++) {
        try {
          const uploadResponse = await cafeAPI.uploadMenuImage(files[i])
  ...
  const handleCoverImageDelete = async (indexToDelete) => {
    ...
    try {
      ...
      // Update cafe profile with new menuImages array
      try {
        const updateResponse = await cafeAPI.updateProfile({
  ...
  const handleSaveName = async () => {
    ...
    try {
      // Update cafe name via API
      const response = await cafeAPI.updateProfile({ name: newName })
      
      if (response?.data?.data?.cafe) {
        // Update local state
        setCafeName(newName)
        setCafeData(prev => prev ? { ...prev, name: newName } : null)
        setShowEditNameDialog(false)
        
        // Refresh cafe data to get latest from backend
        const refreshResponse = await cafeAPI.getCurrentCafe()
        const data = refreshResponse?.data?.data?.cafe || refreshResponse?.data?.cafe
        if (data) {
          setCafeData(data)
          setCafeName(data.name || newName)
        }
      } else {
      ...
      alert(`Failed to update cafe name: ${error.response?.data?.message || error.message || "Please try again."}`)
    }
  }
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-gray-900 font-normal">
              Cafe id: {loading ? "Loading..." : (cafeMongoId && cafeMongoId.length >= 5 ? cafeMongoId.slice(-5) : (cafeId || "N/A"))}
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 pb-6 space-y-3">
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={() => navigate("/cafe/edit-cuisines")}
        ...
        >
          <div className="flex items-start justify-between mb-2">
            ...
            <button
              onClick={() => navigate("/cafe/edit-address")}
              className="text-blue-600 text-sm font-normal hover:text-blue-700 transition-colors ml-4 shrink-0 self-start"
            >
              Edit
            </button>
        ...
        >
          {/* Outlet Timings Card */}
          <button
            onClick={() => navigate("/cafe/outlet-timings")}
          ...
          <button
            onClick={() => navigate("/cafe/contact-details")}
            className="w-full bg-blue-100/50 rounded-lg p-4 border border-blue-300 flex items-center justify-between hover:bg-gray-50 transition-colors"
          >
            <div className="flex items-center gap-3">
