...
export default function ZoneSetup() {
  ...
  useEffect(() => {
    if (cafeData?.location && mapInstanceRef.current && !mapLoading && window.google) {
      const location = cafeData.location
      let lat = null
      let lng = null
    ...
    const infoWindow = new window.google.maps.InfoWindow({
      content: `
        <div style="padding: 8px; max-width: 250px;">
          <strong>Cafe Location</strong><br/>
  ...
  const handleSaveLocation = async () => {
    ...
    try {
      ...
      // Update cafe location
      const response = await cafeAPI.updateProfile({
        location: {
          ...(cafeData?.location || {}),
          latitude: lat,
          longitude: lng,
