...
import { cafeAPI, uploadAPI } from "@/lib/api"
...
export default function HubMenu() {
  ...
  // Cafe info - fetch from backend
  const cafeName = cafeData?.name || ""
  const cafeExpertise = cafeData?.cuisines?.length > 0
    ? cafeData.cuisines.join(", ")
    : ""
  ...
  const fetchMenu = async (showLoading = true) => {
    try {
      ...
      const response = await cafeAPI.getMenu()
    ...
    if (!loadingMenu && menuData.length >= 0) {
      const timeoutId = setTimeout(async () => {
        try {
          ...
          await cafeAPI.updateMenu({ sections: normalizedSections })
  ...
  const fetchAddons = async (showLoading = true) => {
    try {
      ...
      const response = await cafeAPI.getAddons()
  ...
  const handleDeleteAddon = async (addon) => {
    ...
    try {
      await cafeAPI.deleteAddon(addon.id)
  ...
  const handleAvailabilityConfirm = async () => {
    ...
    try {
      ...
      if (switchingOffTarget.type === 'item') {
        ...
        const response = await cafeAPI.scheduleItemAvailability(scheduleData)
  ...
  const handleSaveCategoryName = () => {
    ...
    // Save updated foods
    try {
      localStorage.setItem('cafe_foods', JSON.stringify(updatedFoods))
  ...
  const handleContinueSubCategory = () => {
    ...
    navigate('/cafe/hub-menu/item/new', {
      state: {
  ...
  const handleContinueAddCategory = async () => {
    ...
    try {
      ...
      const response = await cafeAPI.addSection(newCategoryName.trim())

      if (response.data && response.data.success) {
        // Refresh menu data
        const menuResponse = await cafeAPI.getMenu()
  ...
  const handleDeleteCategory = async () => {
    ...
    try {
      ...
      await cafeAPI.updateMenu({ sections: updatedSections })
        ...
        <div className="">
              ...
              <AnimatePresence mode="wait">
                {!isScrolled ? (
                  ...
                  <motion.h1
                    ...
                    title={cafeName}
                  >
                    {cafeName.length > 25 ? `${cafeName.substring(0, 25)}...` : cafeName}
                  </motion.h1>
                )}
              </AnimatePresence>
            </div>
          ...
          <AnimatePresence>
            {!isScrolled && (
              ...
              >
                <h2 className="text-lg font-bold text-gray-900">{cafeName}</h2>
                <p className="text-sm text-gray-600">{cafeExpertise}</p>
              </motion.div>
            )}
          </AnimatePresence>
                        ...
                        {group.items.map((item) => (
                          <div key={item.id} className="border-b border-gray-100 pb-4 last:border-0 last:pb-0">
                            ...
                            <div className="flex items-center justify-center gap-3 mt-4">
                              <button
                                onClick={() => navigate(`/cafe/hub-menu/item/${item.id}`, { state: { item, groupId: group.id } })}
                          ...
                          <div className="space-y-3">
                            {group.items.map((item) => (
                              <div
                                ...
                                onClick={() => {
                                  setIsSearchOpen(false)
                                  navigate(`/cafe/hub-menu/item/${item.id}`, {
