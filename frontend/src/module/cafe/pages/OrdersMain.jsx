...
import { cafeAPI, diningAPI } from "@/lib/api"
import { useCafeNotifications } from "../hooks/useCafeNotifications"
import { jsPDF } from "jspdf"
import autoTable from "jspdf-autotable"

const STORAGE_KEY = "cafe_online_status"
...
function CompletedOrders({ onSelectOrder }) {
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        const response = await cafeAPI.getOrders()
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      try {
        ...
        if (response.data?.success && response.data.data?.orders) {
          // Filter cancelled orders (both cafe and user cancelled)
...
function TableBookings() {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      try {
        const res = await cafeAPI.getCurrentCafe()
        const cafe = res.data?.data?.cafe || res.data?.cafe || res.data?.data
        const cafeId = cafe?._id || cafe?.id

        if (cafeId) {
          const response = await diningAPI.getCafeBookings(cafeId)
...
export default function OrdersMain() {
  ...
  // Cafe notifications hook for real-time orders
  const { newOrder, clearNewOrder, isConnected } = useCafeNotifications()
  ...
  // Fetch cafe verification status
  useEffect(() => {
    const fetchCafeStatus = async () => {
      try {
        const response = await cafeAPI.getCurrentCafe()
        const cafe = response?.data?.data?.cafe || response?.data?.cafe
        if (cafe) {
          setCafeStatus({
          ...
          const completedSteps = cafe.onboarding?.completedSteps || 0
          if (completedSteps < 4) {
            // Onboarding is incomplete, redirect to onboarding page
            const incompleteStep = await checkOnboardingStatus()
            if (incompleteStep) {
              navigate(`/cafe/onboarding?step=${incompleteStep}`, { replace: true })
              return
            }
          }
        }
        ...
        setCafeStatus(prev => ({ ...prev, isLoading: false }))
      }
    }

    fetchCafeStatus()

    // Listen for cafe profile updates
    const handleProfileRefresh = () => {
      fetchCafeStatus()
    }

    window.addEventListener('cafeProfileRefresh', handleProfileRefresh)

    return () => {
      window.removeEventListener('cafeProfileRefresh', handleProfileRefresh)
    }
  }, [navigate])
  ...
  const handleReverify = async () => {
    try {
      ...
      window.dispatchEvent(new Event('cafeProfileRefresh'))
        ...
        if (response.data?.success && response.data.data?.orders) {
          ...
          if (confirmedOrders.length > 0 && !showNewOrderPopupRef.current && !newOrderRef.current) {
            ...
            const orderForPopup = {
              ...
              cafeId: latestConfirmedOrder.cafeId,
              cafeName: latestConfirmedOrder.cafeName,
  ...
  const handleAcceptOrder = async () => {
    ...
    if (orderToAccept?.orderMongoId || orderToAccept?.orderId) {
      try {
        ...
        const response = await cafeAPI.acceptOrder(orderId, prepTime)
  ...
  const handleRejectConfirm = async () => {
    ...
    if (orderToReject?.orderMongoId || orderToReject?.orderId) {
      try {
        ...
        await cafeAPI.rejectOrder(orderId, rejectReason)
  ...
  const handleCancelConfirm = async () => {
    ...
    try {
      ...
      await cafeAPI.rejectOrder(orderId, cancelReason.trim())
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      doc.text(orderToPrint.cafeName || 'Cafe', 105, 30, { align: 'center' })
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col">
      {/* Cafe Navbar - Sticky at top */}
      <div className="sticky top-0 z-50 bg-white">
        <CafeNavbar showNotifications={false} />
      </div>
      ...
      >
        ...
        {/* Verification Pending Card - Show if onboarding is complete (all 4 steps) and cafe is not active */}
        {!cafeStatus.isLoading &&
          !cafeStatus.isActive &&
          cafeStatus.onboarding?.completedSteps === 4 && (
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3, delay: 0.1 }}
              className={`mt-4 mb-4 rounded-2xl shadow-sm px-6 py-4 ${cafeStatus.rejectionReason
                ? 'bg-white border border-red-200'
                : 'bg-white border border-yellow-200'
                }`}
            >
              {cafeStatus.rejectionReason ? (
                <>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="bg-red-50 border border-red-200 rounded-lg p-3 mb-3">
                        ...
                        <div className="text-xs text-red-700 space-y-1">
                          {cafeStatus.rejectionReason.split('\n').filter(line => line.trim()).length > 1 ? (
                            ...
                            <p className="text-red-700">{cafeStatus.rejectionReason}</p>
                          )}
                        </div>
                      </div>
                    </div>
                ...
                <div className="px-4 py-3 bg-white border-b border-gray-200 flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">
                      {(popupOrder || newOrder)?.cafeName || 'Cafe'}
                    </p>
                  </div>
...
function ResendNotificationButton({ orderId, mongoId, onSuccess }) {
  ...
  const handleResend = async (e) => {
    ...
    try {
      ...
      const response = await cafeAPI.resendDeliveryNotification(id);
        ...
        if (remainingMinutes <= 0 && order.status === 'preparing') {
          ...
          if (elapsedSeconds >= totalETASeconds - 2) {
            try {
              ...
              await cafeAPI.markOrderReady(order.mongoId || order.orderId)
