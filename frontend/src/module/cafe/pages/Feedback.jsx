...
const REVIEWS_STORAGE_KEY = "cafe_reviews_data"
  ...
  useEffect(() => {
    const fetchCafeData = async () => {
      try {
        ...
        if (response.data?.success && response.data.data?.cafe) {
  ...
  useEffect(() => {
    const fetchComplaints = async () => {
      ...
      try {
        ...
        const response = await cafeAPI.getComplaints(params)
      ...
      try {
        ...
        const transformedReviews = allOrders
          ...
          .map((order, index) => {
            ...
            // Get outlet/cafe name
            const outlet = order.cafeName || 
                          (cafeData?.name) ||
                          'Cafe'
            ...
            // Count user's orders with this cafe
        ...
        <div className="sticky bg-white top-0 z-40 px-4 py-3 border-b border-gray-200 flex items-center justify-between mb-2">
          <div>
            ...
            <p className="text-md font-semibold text-gray-900 mt-0.5">
              {isLoadingCafe ? "Loading..." : (cafeData?.name || "Cafe")}
            </p>
          </div>
                  ...
                  <div className="space-y-3 pb-6">
                    {complaints.map((complaint) => (
                      <div key={complaint._id} className="bg-white rounded-lg p-4 border border-gray-200">
                        ...
                        {complaint.cafeResponse && (
                          <div className="bg-gray-50 rounded p-3 mt-3">
                            <p className="text-xs font-semibold text-gray-700 mb-1">Your Response:</p>
                            <p className="text-sm text-gray-700">{complaint.cafeResponse}</p>
                          </div>
                        )}
