...
export default function CafeSignIn() {
  ...
  // Redirect to cafe home if already authenticated
  useEffect(() => {
    const isAuthenticated = localStorage.getItem("cafe_authenticated") === "true"
    if (isAuthenticated) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Login with cafe auth endpoint
      const response = await cafeAPI.login(email, password)
      const data = response?.data?.data || response?.data
      
      if (data.accessToken && data.cafe) {
        // Replace old token with new one (handles cross-module login)
        setAuthData("cafe", data.accessToken, data.cafe)
        ...
        >
          ...
          >
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                <Input
                  id="email"
                  type="email"
                  placeholder="test.cafe@gmail.com"
                  value={email}
            ...
            <div className="flex items-center justify-between text-sm">
              ...
              <button
                type="button"
                onClick={() => navigate("/cafe/forgot-password")}
          ...
          <div className="mt-6 text-center text-sm text-gray-600">
            ...
            <button
              onClick={() => navigate("/cafe/signup-email")}
              className="text-primary-orange hover:underline font-medium"
            >
              Sign up
            </button>
