...
import { setAuthData as setCafeAuthData } from "@/lib/utils/auth"
import { checkOnboardingStatus } from "../../utils/onboardingUtils"

export default function CafeOTP() {
  ...
  useEffect(() => {
    ...
    const stored = sessionStorage.getItem("cafeAuthData")
    if (stored) {
  ...
  const handleVerify = async (otpValue = null) => {
    ...
    try {
      ...
      const response = await cafeAPI.verifyOTP(phone, code, purpose, nameToSend, email)

      // Extract cafe and token or special flags (like needsName) from backend response
      const data = response?.data?.data || response?.data

      // If backend says we need a name (cafe not found on login), treat this as a new signup:
      ...
      if (data?.needsName) {
        setAuthData((prev) => {
          ...
          try {
            sessionStorage.setItem("cafeAuthData", JSON.stringify(updated))
          } catch {
      ...
      if (accessToken && cafe) {
        ...
        setCafeAuthData("cafe", accessToken, cafe)
        ...
        setTimeout(async () => {
          ...
          if (authData?.isSignUp) {
            navigate("/cafe/onboarding", { replace: true })
          } else {
            ...
            try {
              ...
              if (incompleteStep) {
                ...
                // Onboarding is complete, go to cafe home
                navigate("/cafe", { replace: true })
              }
            } catch (err) {
              console.error("Failed to check onboarding status:", err)
              // Fallback to cafe home
              navigate("/cafe", { replace: true })
            }
          }
        }, 500)
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      await cafeAPI.sendOTP(phone, purpose, email)
    } catch (err) {
      const message =
        err?.response?.data?.message ||
        err?.response?.data?.error ||
