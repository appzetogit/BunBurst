...
export default function CafeGoogleCallback() {
  ...
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        // Redirect to cafe home after short delay
        setTimeout(() => {
          navigate("/cafe")
        }, 1200)
      } catch (err) {
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-lg p-6 space-y-6">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-gray-600">
            {status === "loading" &&
              `Connecting your ${provider || "Google"} account to ${companyName} Cafe Panel...`}
