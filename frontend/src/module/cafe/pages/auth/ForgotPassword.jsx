...
export default function CafeForgotPassword() {
  ...
  const handleEmailSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await cafeAPI.sendOTP(null, "reset-password", email)
      setStep(2)
      setResendTimer(60)
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      await cafeAPI.verifyOTP(null, otpCode, "reset-password", null, email)
      setStep(3)
    } catch (err) {
      const message =
        err?.response?.data?.message ||
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    setIsLoading(true)
    try {
      const response = await cafeAPI.resetPassword(email, otp.join(""), newPassword)
        ...
        <div className="absolute inset-0 flex items-center text-white pointer-events-none">
          <div className="bg-primary-orange/80 rounded-r-full py-10 xl:py-20 pl-10 xl:pl-14 pr-10 xl:pr-20 max-w-[70%] shadow-xl backdrop-blur-[1px]">
            ...
            <p className="text-base xl:text-lg opacity-95 max-w-xl">
              Follow the steps to securely reset your cafe panel password.
            </p>
          </div>
        </div>
      </div>
