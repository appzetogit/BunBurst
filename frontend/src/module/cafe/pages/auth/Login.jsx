...
export default function CafeLogin() {
  ...
  const handleSendOTP = async () => {
    ...
    try {
      ...
      await cafeAPI.sendOTP(fullPhone, "login")
      ...
      sessionStorage.setItem("cafeAuthData", JSON.stringify(authData))
  ...
  const handleSendEmailOTP = async () => {
    ...
    try {
      ...
      await cafeAPI.sendOTP(null, "login", formData.email)
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const response = await cafeAPI.firebaseGoogleLogin(idToken)
      ...
      // Store auth data for cafe module using utility function
      setAuthData("cafe", accessToken, cafe)

      // Notify any listeners that auth state has changed
      window.dispatchEvent(new Event("cafeAuthChanged"))

      // Navigate to cafe home
      navigate("/cafe")
    } catch (error) {
