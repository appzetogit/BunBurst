...
export default function CafeStatus() {
  ...
  useEffect(() => {
    const loadOutletTimings = () => {
      try {
        const saved = localStorage.getItem("cafe_outlet_timings")
        if (saved) {
  ...
  // Check if cafe is currently open based on timings
  useEffect(() => {
    if (!cafeData) return
    ...
    const checkIfOpen = () => {
      ...
      const STORAGE_KEY = "cafe_outlet_timings"
      ...
      const openDays = cafeData.openDays || []
      ...
      const deliveryTimings = cafeData.deliveryTimings
  ...
  useEffect(() => {
    const loadDeliveryStatus = async () => {
      try {
        ...
        if (cafe?.isAcceptingOrders !== undefined) {
          setDeliveryStatus(cafe.isAcceptingOrders)
          // Sync localStorage with backend
          localStorage.setItem('cafe_online_status', JSON.stringify(cafe.isAcceptingOrders))
          // Dispatch event to update navbar
          window.dispatchEvent(new CustomEvent('cafeStatusChanged', { 
            detail: { isOnline: cafe.isAcceptingOrders } 
          }))
        } else {
          // Fallback to localStorage
          const savedStatus = localStorage.getItem('cafe_online_status')
          if (savedStatus !== null) {
  ...
  const handleDeliveryStatusChange = async (checked) => {
    ...
    try {
      ...
      localStorage.setItem('cafe_online_status', JSON.stringify(checked))
      
      // Update backend
      try {
        await cafeAPI.updateDeliveryStatus(checked)
  ...
  const getCurrentDayTimings = () => {
    ...
    if (cafeData?.deliveryTimings) {
      const openingTime = formatTime12Hour(cafeData.deliveryTimings.openingTime)
      const closingTime = formatTime12Hour(cafeData.deliveryTimings.closingTime)
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 min-w-0">
                <h2 className="text-base font-bold text-gray-900 mb-1">
                  {loading ? "Loading..." : (cafeData?.name || "Cafe")}
                </h2>
                <p className="text-sm text-gray-500">
                  {loading ? "Loading..." : (
                    <>
                      {cafeData?.id ? `ID: ${String(cafeData.id).slice(-5)}` : ""}
                      {cafeData?.location && formatAddress(cafeData.location) ? (
                        <> | {formatAddress(cafeData.location)}</>
                      ) : ""}
                    </>
                  )}
                </p>
              ...
              <button
                onClick={() => {
                  // Navigate to cafe settings
                  navigate("/cafe/explore")
                }}
  ...
  {!isWithinTimings && cafeData && !isDayClosed && (
