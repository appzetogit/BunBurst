...
export default function ExploreMore() {
  ...
  const STORAGE_KEY = "cafe_schedule_off"
  ...
  // Get user data from cafe data
  const userData = cafeData ? {
    name: cafeData.ownerName || cafeData.name || "Cafe Owner",
    phone: cafeData.ownerPhone || cafeData.phone || "N/A",
    email: cafeData.ownerEmail || cafeData.email || "N/A",
    role: "OWNER",
    profileImage: cafeData.profileImage
  } : {
    name: "Loading...",
    phone: "",
    email: "",
  ...
  // Get cafe display data
  const cafeDisplayName = cafeData?.name || "Loading..."
  const cafeDisplayAddress = cafeData?.location ? formatAddress(cafeData.location) : ""
  ...
  const handleLogout = async () => {
    ...
    try {
      ...
      // Sign out from Firebase if cafe logged in via Google
      try {
      ...
      // Clear cafe module authentication data
      clearModuleAuth("cafe")

      // Clear any onboarding data from localStorage
      localStorage.removeItem("cafe_onboarding")
      localStorage.removeItem("cafe_accessToken")
      localStorage.removeItem("cafe_authenticated")
      localStorage.removeItem("cafe_user")

      // Clear sessionStorage
      sessionStorage.removeItem("cafeAuthData")

      // Dispatch auth change event to notify other components
      window.dispatchEvent(new Event("cafeAuthChanged"))

      // Small delay for UX, then navigate to welcome page
      setTimeout(() => {
        navigate("/cafe/welcome", { replace: true })
      }, 300)
    } catch (error) {
  ...
  const handleLogoutAllDevices = async () => {
    ...
    try {
      ...
      // Clear auth for all modules (admin, cafe, delivery, user)
      clearAuthData()
  ...
  const manageOutletItems = [
    { id: 1, label: "Outlet info", icon: Info, route: "/cafe/outlet-info" },
    { id: 2, label: "Outlet timings", icon: Clock, route: "/cafe/outlet-timings" },
    { id: 3, label: "Dining Reservations", icon: Calendar, route: "/cafe/reservations" },
    { id: 4, label: "Manage staff", icon: Users, route: "/cafe/contact-details" },
  ]

  const settingsItems = [
    { id: 3, label: "Delivery settings", icon: Truck, route: "/cafe/delivery-settings" },
    { id: 4, label: "Zone Setup", icon: MapPin, route: "/cafe/zone-setup" },
  ]

  const ordersItems = [
    { id: 1, label: "Order history", icon: FileText, route: "/cafe/orders/all" },
    { id: 2, label: "Complaints", icon: Star, route: "/cafe/feedback?tab=complaints" },
    { id: 3, label: "Reviews", icon: MessageSquare, route: "/cafe/feedback" },
  ]

  const helpItems = [
    { id: 1, label: "Help centre", icon: HelpCircle, route: "/cafe/help-centre" },
    { id: 3, label: "Share your feedback", icon: Edit, route: "/cafe/Share-Feedback" },
  ]

  const accountingItems = [
    { id: 1, label: "Payout", icon: IndianRupee, route: "/cafe/hub-finance" },
    { id: 2, label: "Invoices", icon: Receipt, route: "/cafe/hub-finance?tab=invoices" },
  ]
        ...
        >
          <Card className="bg-white border-gray-200 py-3 mb-6 rounded-lg shadow-0">
            <CardContent className="px-4">
              <button
                onClick={() => navigate("/cafe/switch-outlet")}
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-base font-bold text-gray-900 mb-1">
                      {loadingCafe ? "Loading..." : userData.name}
                    </h3>
                    {userData.phone && (
