...
export default function ToHub() {
  ...
  const quickLinks = [
    ...
    { id: "order-history", label: "Order history", icon: FaHistory, route: "/cafe/orders/all" },
    { id: "complaints", label: "Complaints", icon: FaExclamationTriangle, route: "/cafe/feedback?tab=complaints" },
    { id: "reviews", label: "Reviews", icon: FaStar, route: "/cafe/feedback" },
    { id: "feedback", label: "Share your feedback", icon: FaCommentDots, route: "/cafe/Share-Feedback" },
    { id: "zone-setup", label: "Zone Setup", icon: MapPin, route: "/cafe/zone-setup" },
    { id: "settings", label: "Settings", icon: FaCog, route: "/cafe/delivery-settings" },
    { id: "show-all", label: "Show all", icon: FaThLarge, route: "/cafe/explore" },
  ]
  ...
  useEffect(() => {
    if (!cafeData) return // Don't fetch if cafe data is not loaded yet
    fetchOrdersAndUpdateChart(selectedDateRange)
  }, [cafeData, selectedDateRange, fetchOrdersAndUpdateChart])
        ...
        <div className="sticky bg-white top-0 z-40 px-4 py-3 border-b border-gray-200 flex items-center justify-between">
          <div>
            ...
            <p className="text-md font-semibold text-gray-900 mt-0.5">
              {loadingCafe ? "Loading..." : cafeData?.name || "Cafe"}
            </p>
          </div>
          ...
          <div className="flex items-center">
            ...
            <button
              ...
              onClick={() => navigate("/cafe/help-centre")}
            >
