...
export default function HubFinance() {
  ...
  useEffect(() => {
    const fetchFinanceData = async () => {
      try {
        setLoading(true)
        const response = await cafeAPI.getFinance()
  ...
  // Fetch cafe data for header display
  useEffect(() => {
    // Use cafe data from financeData if available, otherwise fetch separately
    if (financeData?.cafe) {
      setCafeData(financeData.cafe)
    } else {
      const fetchCafeData = async () => {
        try {
          const response = await cafeAPI.getCafeByOwner()
          const data = response?.data?.data?.cafe || response?.data?.cafe || response?.data?.data
          if (data) {
            setCafeData({
              name: data.name,
              cafeId: data.cafeId || data._id,
  ...
  // Format cafe ID to REST###### format (e.g., REST005678)
  const formatCafeId = (cafeId) => {
    if (!cafeId) return ''
    
    // Extract numeric part from the end (e.g., "REST-1768762345335-5678" -> "5678")
    const strId = String(cafeId)
  ...
  const getReportData = () => {
    const cafeName = financeData?.cafe?.name || "Cafe"
    const cafeId = financeData?.cafe?.cafeId || "N/A"
    ...
    return `
      ...
      <body>
        <div class="header">
          ...
          <p>${reportData.cafeName}</p>
          <p>ID: ${reportData.cafeId}</p>
            ...
            <table class="orders-table">
              ...
              <tbody>
                ${reportData.allOrders.map(order => {
                  ...
                  const earning = order.payout || order.cafeEarning || 0
              ...
              <tfoot>
                <tr style="background-color: #e8f5e9; font-weight: bold;">
                  ...
                  <td colspan="2">₹${reportData.allOrders.reduce((sum, order) => sum + (order.payout || order.cafeEarning || 0), 0).toFixed(2)}</td>
                </tr>
              </tfoot>
            </table>
          ` : `
        ...
        <div class="footer">
          ...
          <p>Total Orders: ${reportData.allOrders?.length || 0} | Total Earnings: ₹${reportData.allOrders?.reduce((sum, order) => sum + (order.payout || order.cafeEarning || 0), 0).toFixed(2) || '0.00'}</p>
        </div>
      </body>
      </html>
    `
        ...
        <div className="flex items-center justify-between">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <p className="text-lg font-bold text-gray-900 truncate">
                  {cafeData?.name || financeData?.cafe?.name || "Cafe"}
                </p>
                <ChevronDown className="w-4 h-4 text-gray-600 flex-shrink-0" />
              </div>
              <p className="text-xs text-gray-600 mt-0.5">
                {(() => {
                  const cafeId = cafeData?.cafeId || financeData?.cafe?.cafeId
                  const address = cafeData?.address || financeData?.cafe?.address || ''
                  const parts = []
                  if (cafeId) {
                    const formattedId = formatCafeId(cafeId)
          ...
          <div className="flex items-center gap-1 ml-2">
            <button
              ...
              onClick={() => navigate("/cafe/withdrawal-history")}
            ...
            <button
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
              onClick={() => navigate("/cafe/notifications")}
            >
              <Bell className="w-5 h-5 text-gray-700" />
            </button>
            <button
              className="p-2 hover:bg-gray-100 rounded-full transition-colors"
              onClick={() => navigate("/cafe/explore")}
            >
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const response = await cafeAPI.createWithdrawalRequest(amount)
                        if (response.data?.success) {
                          alert('Withdrawal request submitted successfully!')
                          setShowWithdrawalModal(false)
                          setWithdrawalAmount('')
                          // Refresh finance data
                          const financeResponse = await cafeAPI.getFinance()
