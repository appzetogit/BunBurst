...
export default function AllOrdersPage() {
  ...
  const transformOrder = useCallback((order) => {
    ...
    // Get cafe name
    const cafeName = cafeData?.name || order.cafeId?.name || 'Cafe'
    ...
    if (status === 'REJECTED' && order.rejectionReason) {
      reason = `Rejected by Cafe: ${order.rejectionReason}`
    } else if (status === 'CANCELLED' && order.cancellationReason) {
      reason = `Cancelled by ${order.cancelledBy === 'customer' ? 'customer' : 'cafe'}: ${order.cancellationReason}`
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await cafeAPI.getOrders(params)
    ...
    fetchOrders()
  }, [startDate, endDate, cafeData, transformOrder])
          ...
          <AnimatePresence mode="popLayout">
            {filteredOrders.map((order, index) => (
              <motion.div
                ...
                onClick={() => navigate(`/cafe/orders/${order.id}`)}
