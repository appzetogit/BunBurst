...
const ONBOARDING_STORAGE_KEY = "cafe_onboarding_data"
...
export default function CafeOnboarding() {
  ...
  useEffect(() => {
    ...
    if (localData) {
      if (localData.step1) {
        setStep1({
          cafeName: localData.step1.cafeName || "",
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true)
        const res = await api.get("/cafe/onboarding")
        const data = res?.data?.data?.onboarding
        if (data) {
          if (data.step1) {
            setStep1((prev) => ({
              cafeName: data.step1.cafeName || "",
        ...
        if (err?.response?.status === 401) {
          ...
          // The error might be because cafe is not yet active (pending verification)
        } else {
  ...
  const handleNext = async () => {
    ...
    try {
      if (step === 1) {
        ...
        await api.put("/cafe/onboarding", payload)
        setStep(2)
      } else if (step === 2) {
        const menuUploads = []
        ...
        if (step2.profileImage instanceof File) {
          try {
            profileUpload = await handleUpload(step2.profileImage, "appzeto/cafe/profile")
            // Verify upload was successful and has valid URL
        ...
        const response = await api.put("/cafe/onboarding", payload)
        ...
        // After step2, also update cafe schema with step2 data
        ...
        if (step3.panImage instanceof File) {
          try {
            panImageUpload = await handleUpload(step3.panImage, "appzeto/cafe/pan")
            // Verify upload was successful and has valid URL
        ...
        if (step3.gstRegistered) {
          if (step3.gstImage instanceof File) {
            try {
              gstImageUpload = await handleUpload(step3.gstImage, "appzeto/cafe/gst")
              // Verify upload was successful and has valid URL
        ...
        if (step3.fssaiImage instanceof File) {
          try {
            fssaiImageUpload = await handleUpload(step3.fssaiImage, "appzeto/cafe/fssai")
            // Verify upload was successful and has valid URL
        ...
        setTimeout(() => {
          // Navigate to cafe home page after onboarding completion
