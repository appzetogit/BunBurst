    ...
    const fetchCafe = async () => {
      try {
        ...
        if (response?.data?.success && response?.data?.data?.cafe) {
          setCafe(response.data.data.cafe)
        }
      } catch (error) {
      ...
      <div className="sticky top-0 z-40 bg-white border-b border-gray-200">
        <div className="px-4 py-3">
          <div className="flex items-center gap-3">
            <button
              onClick={() => navigate("/cafe/hub-growth")}
            ...
            <div className="flex-1">
              <div className="flex items-center gap-2">
                <h1 className="text-lg font-bold text-gray-900">
                  {loadingCafe ? "Loading..." : (cafe?.name || "Cafe")}
                </h1>
                <ChevronDown className="w-4 h-4 text-gray-500" />
              </div>
              <p className="text-xs text-gray-500 mt-0.5">
                {(() => {
                  if (loadingCafe) return "Loading..."
                  
                  if (!cafe?.location) return "Location not available"
              ...
              <div className="">
                {offerGoals.map((goal) => (
                  <motion.div
                    ...
                    whileTap={{ scale: 0.98 }}
                    onClick={() => {
                      navigate(`/cafe/hub-growth/create-offers/${goal.id}`)
                    }}
