...
export default function ItemDetailsPage() {
  ...
  // Fetch categories from cafe-specific API
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        setLoadingCategories(true)
        const response = await cafeAPI.getCategories()
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        for (let i = 0; i < filesToUpload.length; i++) {
          ...
          try {
            ...
            const uploadResponse = await uploadAPI.uploadMedia(file, {
              folder: 'appzeto/cafe/menu-items'
            })
      ...
      const itemDataToSave = {
        ...
        categoryId: categoryId || itemData?.categoryId || null,
      ...
      const updateResponse = await cafeAPI.updateMenu({ sections })
      ...
      if (updateResponse.data?.success) {
        ...
        navigate("/cafe/hub-menu", { replace: true })
