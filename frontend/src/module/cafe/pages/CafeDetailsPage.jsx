...
import { getCafeData } from "../utils/cafeManagement"

export default function CafeDetailsPage() {
  const navigate = useNavigate()
  const [activeCategory, setActiveCategory] = useState("all")
  const [showMenu, setShowMenu] = useState(false)
  const [cafeData, setCafeData] = useState(() => getCafeData())
  ...
  // Load cafe data and listen for updates
  useEffect(() => {
    const refreshCafeData = () => {
      setCafeData(getCafeData())
    }

    // Initial load
    refreshCafeData()

    // Listen for cafe data changes
    window.addEventListener('cafeDataUpdated', refreshCafeData)
    window.addEventListener('storage', refreshCafeData)

    return () => {
      window.removeEventListener('cafeDataUpdated', refreshCafeData)
      window.removeEventListener('storage', refreshCafeData)
    }
  }, [])
  ...
  return (
    <div className="min-h-screen bg-page-bg overflow-x-hidden">
      ...
      <div className="relative w-full h-[250px] md:h-[300px] overflow-hidden">
        <img 
          src={cafeData.cover || "https://images.unsplash.com/photo-1512058564366-18510be2db19?w=800&h=400&fit=crop"}
          alt="Cafe Hero"
          className="w-full h-full object-cover"
        />
        
        {/* Cafe Info Card Overlay */}
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between mb-2">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900">
                  {cafeData.cafeName?.english || "Hungry Puppets"}
                </h2>
                <motion.button
                  whileHover={{ scale: 1.1 }}
                  whileTap={{ scale: 0.9 }}
                  onClick={() => navigate("/cafe/edit")}
              ...
              <p className="text-gray-600 text-sm md:text-base mb-2">
                {cafeData.address || "House: 00, Road: 00, Test City"}
              </p>
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600 text-sm md:text-base underline">
                  {cafeData.totalRatings || 3} Ratings
                </span>
              </div>
            </div>
          </div>
          ...
          {filteredFoodItems.length === 0 ? (
            ...
            >
              <Card 
                ...
                onClick={() => navigate(`/cafe/food/${item.id}`)}
              >
