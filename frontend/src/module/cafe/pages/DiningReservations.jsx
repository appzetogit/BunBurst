...
import { diningAPI, cafeAPI } from "@/lib/api"
...
export default function DiningReservations() {
    ...
    useEffect(() => {
        const fetchAll = async () => {
            try {
                // First get the current cafe
                const resResponse = await cafeAPI.getCurrentCafe()
                if (resResponse.data.success) {
                    const resData = resResponse.data?.data?.cafe || resResponse.data?.cafe || resResponse.data?.data

                    const cafeId = resData?._id || resData?.id

                    if (cafeId) {
                        setCafe(resData)
                        // Then get its bookings
                        const bookingsResponse = await diningAPI.getCafeBookings(cafeId)
    ...
    const handleStatusUpdate = async (bookingId, newStatus) => {
        try {
            const response = await diningAPI.updateBookingStatusCafe(bookingId, newStatus)
