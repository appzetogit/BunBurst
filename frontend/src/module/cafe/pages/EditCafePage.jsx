...
import { getCafeData, updateCafeData } from "../utils/cafeManagement"
...
export default function EditCafePage() {
  ...
  const [formData, setFormData] = useState(() => {
    const savedData = getCafeData()
    return {
      cafeName: savedData.cafeName || {
  ...
  useEffect(() => {
    ...
    window.addEventListener('cafeDataUpdated', refreshData)
    window.addEventListener('storage', refreshData)

    return () => {
      window.removeEventListener('cafeDataUpdated', refreshData)
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.cafeName.english || !formData.address || !formData.phoneNumber) {
    ...
    try {
      updateCafeData(formData)
      // Navigate back to cafe details page
          ...
          <Card className="bg-white shadow-sm border-0">
            <CardContent className="p-4 md:p-6">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.cafeName[activeLanguage]}
