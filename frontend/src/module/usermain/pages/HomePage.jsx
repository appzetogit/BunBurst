...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-background overflow-x-hidden pb-20 text-foreground">
      {/* Toast Notification */}
      <Toast show={toast.show} message={toast.message} />
      {/* Top Header - Primary Bar */}
      <div className="bg-card border-b border-border w-full px-4 py-3 relative">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            <MapPin className="w-5 h-5 text-primary flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-muted-foreground text-xs font-medium mb-0.5">Your Location</p>
              <div className="flex items-center gap-1">
                ...
                <svg className="w-2.5 h-2.5 text-foreground flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="px-4 py-4 relative">
        <div className="bg-muted rounded-2xl flex items-center gap-3 px-4 py-3 shadow-sm mx-auto w-full relative z-10 border border-border">
          <Search className="w-5 h-5 text-muted-foreground" />
          <input
            type="text"
            placeholder={placeholderTexts[currentPlaceholderIndex]}
            className="flex-1 bg-transparent outline-none text-sm text-foreground placeholder-muted-foreground transition-all duration-300"
          />
      ...
      <div className="px-4 mb-6">
        <div
          className="relative rounded-xl overflow-hidden h-36 md:h-48 cursor-pointer ring-1 ring-border"
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center px-4 py-3">
                <div className="flex-1 z-10">
                  ...
                  >
                    <Button
                      className="bg-primary hover:bg-primary/90 text-primary-foreground border-none rounded-full px-3 md:px-5 py-1.5 md:py-2 text-xs md:text-sm font-bold transition-colors"
                    >
                      Order Now
                    </Button>
                    {carouselSlides[currentSlide].rating && (
                      <span className="text-white text-xs md:text-sm font-semibold bg-white/10 backdrop-blur-sm border border-white/20 px-2 md:px-3 py-1 md:py-1.5 rounded-full">
                        {carouselSlides[currentSlide].rating}
                      </span>
                    )}
                  </motion.div>
              ...
              <motion.div
                ...
                className="absolute bottom-2 md:bottom-4 right-2 md:right-4 bg-accent text-white rounded-full px-2 md:px-3 py-1 md:py-1.5 text-[10px] md:text-xs font-bold shadow-lg"
              >
          ...
          <div className="absolute bottom-2 md:bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-2 z-20">
            {carouselSlides.map((_, index) => (
              <button
                ...
                className={`h-1.5 md:h-2 rounded-full transition-all duration-300 ${index === currentSlide ? 'bg-primary w-5 md:w-6' : 'bg-white/30 w-1.5 md:w-2 hover:bg-white/50'
                  }`}
              />
            ))}
          </div>
      ...
      <div className="px-4 mb-6">
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-base md:text-lg font-bold text-foreground">What's on Your Mind?</h3>
          <button
        ...
        <div className="overflow-hidden -mx-4">
          <div className="flex gap-3 animate-scroll">
            ...
            {[...categories, ...categories].map((category, index) => (
              ...
              >
                <div className="w-20 h-20 rounded-full overflow-hidden border-2 border-border p-0.5">
                  <img
                    src={category.image}
                    alt={category.name}
                ...
                <p className="text-[10px] font-medium text-muted-foreground text-center leading-tight">{category.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="px-4 mb-6">
        <h3 className="text-lg font-bold text-primary mb-1">Today's Trends</h3>
        <p className="text-sm text-muted-foreground mb-4">Here's what you might like to taste</p>
        ...
        <div className="grid grid-cols-2 gap-3">
          {trendsItems.map((item) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className="bg-card border border-border rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow cursor-pointer flex flex-col h-full"
            ...
            >
              ...
              <div className="relative flex-shrink-0">
                ...
                />
                {/* Price Badge */}
                <div className="absolute top-1.5 left-1.5 bg-black/80 backdrop-blur-sm border border-primary/50 rounded-full px-2 py-0.5">
                  <span className="text-[10px] font-bold text-primary">{item.price}</span>
                </div>
                {/* Favorite Icon */}
                <button
                  className="absolute top-1.5 right-1.5 p-1 bg-black/50 rounded-full hover:bg-black/70 transition-colors z-10"
                  onClick={(e) => {
                    e.stopPropagation()
                ...
                >
                  <Heart
                    className={`w-3.5 h-3.5 transition-all ${isInWishlist(item, 'food')
              ...
              <div className="p-3 flex flex-col flex-1">
                ...
                <div className="flex items-center justify-between gap-2 mb-2">
                  <h4 className="text-xs font-bold text-card-foreground flex-1 line-clamp-2">{item.name}</h4>
                  <div className="flex items-center gap-0.5 flex-shrink-0">
                    <Star className="w-3 h-3 fill-primary text-primary" />
                    <span className="text-[10px] font-semibold text-card-foreground">{item.rating}</span>
                  </div>
                </div>

                {/* Add Button */}
                <Button
                  className="w-full bg-primary hover:bg-primary/90 text-primary-foreground text-xs font-bold py-1.5 rounded-lg"
                  onClick={(e) => {
                    e.stopPropagation()
      ...
      <div className="px-4 mb-6">
        <motion.div
          ...
          className="bg-card border border-border rounded-2xl overflow-hidden shadow-md relative min-h-[140px] md:min-h-[180px]"
        >
          <div className="flex items-center gap-3 md:gap-6 p-4 md:p-8 h-full">
              ...
              <div className="absolute top-1 left-1/2 -translate-x-1/2 w-16 h-16 md:w-28 md:h-28 z-10">
                <div className="relative w-full h-full">
                  ...
                  <div className="absolute inset-0 rounded-t-full overflow-hidden">
                    ...
                    <div className="absolute top-0 left-0 w-1/2 h-full bg-card rounded-tl-full"></div>
                  </div>
                  {/* Umbrella pole */}
              ...
              <div className="absolute bottom-6 md:bottom-12 left-1/2 -translate-x-1/2 w-14 h-14 md:w-24 md:h-24 bg-white rounded-full shadow-lg flex items-center justify-center z-20">
                ...
                <div className="relative">
                  <div className="w-7 h-7 md:w-12 md:h-12 bg-green-500 rounded-full flex items-center justify-center">
                    <div className="w-4 h-4 md:w-7 md:h-7 bg-accent rounded-full"></div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col items-center gap-2 md:gap-3 flex-shrink-0">
              ...
              <div className="relative">
                <MapPin className="w-6 h-6 md:w-10 md:h-10 text-accent" fill="currentColor" />
              </div>
              {/* See Location Button */}
              <Button
                onClick={() => {
                ...
                className="bg-primary hover:bg-primary/90 text-primary-foreground font-semibold px-4 py-2 md:px-8 md:py-3 rounded-lg text-sm md:text-base shadow-md transition-all hover:scale-105 whitespace-nowrap"
              >
                See Location
              </Button>
            </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t border-border shadow-lg z-50">
        <div className="flex items-center justify-around py-2 px-4">
          ...
          >
            ...
            <span className="text-xs text-muted-foreground font-medium">Wishlist</span>
          </button>
          <button className="flex flex-col items-center gap-1 p-2 -mt-8">
            <div className="bg-card rounded-full p-3 shadow-lg border-2 border-border">
