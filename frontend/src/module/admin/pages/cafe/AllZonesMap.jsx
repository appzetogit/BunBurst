...
export default function AllZonesMap() {
  ...
  useEffect(() => {
    if (!mapLoading && mapInstanceRef.current && window.google) {
      if (zones.length > 0 && cafes.length > 0) {
        drawAllZonesOnMap(window.google, mapInstanceRef.current)
      }
      if (cafes.length > 0) {
        drawCafeMarkers(window.google, mapInstanceRef.current)
      }
    }
  }, [zones, mapLoading, cafes])
  ...
  // Draw cafe markers on the map
  const drawCafeMarkers = (google, map) => {
    ...
    cafes.forEach(cafe => {
      ...
      // Get coordinates from cafe location
      let lat = null
      let lng = null
      ...
      } else if (cafe.location.latitude && cafe.location.longitude) {
        lat = parseFloat(cafe.location.latitude)
        lng = parseFloat(cafe.location.longitude)
      }
      ...
      const infoWindow = new google.maps.InfoWindow({
        content: `
          <div style="padding: 12px; min-width: 200px;">
            ...
            <div style="font-size: 13px; color: #64748b; line-height: 1.6;">
              ${cafe.location?.formattedAddress || cafe.location?.address || cafe.location?.area || 'Location not specified'}
            </div>
            ${cafe.ownerName ? `
              <div style="margin-top: 8px; font-size: 12px; color: #94a3b8;">
                <strong>Owner:</strong> ${cafe.ownerName}
              </div>
            ` : ''}
          </div>
        `
          ...
          {!mapLoading && (
            <div className="mt-4 p-4 bg-slate-50 rounded-lg border border-slate-200">
              ...
              <div className="text-xs text-slate-600 space-y-1">
                ...
                {cafes.length > 0 && (
                  <p>
                    Click on any <span className="font-semibold text-red-600">red marker</span> to view cafe name and details. Total cafes: <strong>{cafes.length}</strong>
                  </p>
                )}
              </div>
            </div>
