...
export default function CafeCommission() {
  const [searchQuery, setSearchQuery] = useState("")
  const [commissions, setCommissions] = useState([])
  const [approvedCafes, setApprovedCafes] = useState([])
  ...
  const filteredCommissions = useMemo(() => {
    ...
    return commissions.filter(commission =>
      commission.cafeName?.toLowerCase().includes(query) ||
      commission.cafeId?.toLowerCase().includes(query) ||
      commission.cafe?.name?.toLowerCase().includes(query)
    )
  ...
  const fetchCommissions = async () => {
    try {
      setLoading(true)
      const response = await adminAPI.getCafeCommissions({})
  ...
  const fetchApprovedCafes = async () => {
    try {
      const response = await adminAPI.getApprovedCafes({ limit: 1000 })
      
      let cafesData = null
      if (response?.data?.success && response?.data?.data?.cafes) {
        cafesData = response.data.data.cafes
      } else if (response?.data?.data?.cafes) {
        cafesData = response.data.data.cafes
      } else if (response?.data?.cafes) {
        cafesData = response.data.cafes
      }
      
      if (cafesData && Array.isArray(cafesData)) {
        setApprovedCafes(cafesData)
      } else {
        setApprovedCafes([])
      }
    } catch (error) {
  ...
  const handleToggleStatus = async (commission) => {
    try {
      await adminAPI.toggleCafeCommissionStatus(commission._id)
  ...
  const handleEdit = async (commission) => {
    try {
      setLoading(true)
      const response = await adminAPI.getCafeCommissionById(commission._id)
      ...
      if (commissionData) {
        ...
        setSelectedCafe(commissionData.cafe)
        
        // Handle cafe ID - it can be an object with _id or just an ID string
        let cafeId = ""
        if (commissionData.cafe) {
          if (typeof commissionData.cafe === 'object' && commissionData.cafe._id) {
            cafeId = commissionData.cafe._id
          } else if (typeof commissionData.cafe === 'string') {
            cafeId = commissionData.cafe
          } else {
            cafeId = commissionData.cafeId || commissionData.cafe?._id || ""
          }
        } else {
          // Fallback to cafeId field if cafe object is not populated
          cafeId = commissionData.cafeId || commissionData.cafe || ""
        }
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true)
      await adminAPI.deleteCafeCommission(selectedCommission._id)
      await fetchCommissions()
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (selectedCommission) {
        await adminAPI.updateCafeCommission(selectedCommission._id, payload)
        toast.success('Commission updated successfully')
      } else {
        await adminAPI.createCafeCommission(payload)
  ...
  return (
    <div className="p-4 lg:p-6 bg-slate-50 min-h-screen">
                        ...
                        {visibleColumns.cafe && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-medium text-blue-600">
                              {commission.cafeName || commission.cafe?.name || '-'}
                            </span>
                          </td>
                        )}
                        {visibleColumns.cafeId && (
                          <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm text-slate-700">{commission.cafeId || '-'}</span>
                          </td>
                        )}
                        {visibleColumns.defaultCommission && (
                          <td className="px-6 py-4 whitespace-nowrap">
      ...
      {/* Cafe Selection Dialog */}
      <Dialog open={isCafeSelectOpen} onOpenChange={setIsCafeSelectOpen}>
                ...
                .map((cafe) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-sm text-slate-900">{cafe.name}</p>
                        <p className="text-xs text-slate-500 mt-0.5">{cafe.cafeId}</p>
                      </div>
                      <Building2 className="w-4 h-4 text-slate-400" />
                    </div>
                  </button>
                ))}
              {filteredCafes.filter(r => !r.hasCommissionSetup).length === 0 && (
          ...
          <div className="space-y-4 px-6 py-4">
            ...
            {selectedCafe && (
              <div className="p-3 bg-blue-50 rounded-lg border border-blue-100">
                <p className="font-semibold text-sm text-slate-900">{selectedCafe.name}</p>
                <p className="text-xs text-slate-600 mt-0.5">{selectedCafe.cafeId}</p>
              </div>
            )}
        ...
        <DialogContent className="max-w-md bg-white">
          ...
          <div className="py-4">
            <p className="text-sm text-slate-700">
              Are you sure you want to delete commission for "{selectedCommission?.cafeName || selectedCommission?.cafe?.name}"? This action cannot be undone.
            </p>
          </div>
          <DialogFooter>
            <button
