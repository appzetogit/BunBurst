...
import { cafeVATReportDummy, cafeVATStats } from "../../data/cafeVATReportDummy"
...
export default function CafeVATReport() {
  const [searchQuery, setSearchQuery] = useState("")
  const [reports, setReports] = useState(cafeVATReportDummy)
  ...
  const filteredReports = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(report =>
        report.cafeName?.toLowerCase().includes(query)
      )
    }

    if (filters.cafe !== "All Cafes") {
      result = result.filter(r => r.cafeName === filters.cafe)
    }
  ...
  const handleExport = (format) => {
    ...
    const headers = [
      { key: "sl", label: "SI" },
      { key: "cafeName", label: "Cafe Info" },
      { key: "totalOrder", label: "Total Order" },
      { key: "totalOrderAmount", label: "Total Order Amount" },
      { key: "taxAmount", label: "Tax Amount" },
    ]
    switch (format) {
      case "csv": exportReportsToCSV(filteredReports, headers, "cafe_vat_report"); break
      case "excel": exportReportsToExcel(filteredReports, headers, "cafe_vat_report"); break
      case "pdf": exportReportsToPDF(filteredReports, headers, "cafe_vat_report", "Cafe VAT Report"); break
      case "json": exportReportsToJSON(filteredReports, "cafe_vat_report"); break
    }
  }
  ...
  const activeFiltersCount = (filters.dateRange ? 1 : 0) + (filters.cafe !== "All Cafes" ? 1 : 0)
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-blue-600">{cafeVATStats.totalOrders}</p>
                <p className="text-lg font-semibold text-blue-600 mt-1">{cafeVATStats.totalOrders}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-green-600">{cafeVATStats.totalOrderAmount}</p>
                <p className="text-lg font-semibold text-green-600 mt-1">{cafeVATStats.totalOrderAmount}</p>
              </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                ...
                <p className="text-3xl font-bold text-red-600">{cafeVATStats.totalTaxAmount}</p>
                <p className="text-lg font-semibold text-red-600 mt-1">{cafeVATStats.totalTaxAmount}</p>
              </div>
                  ...
                  {filteredReports.map((report) => (
                    <tr key={report.sl} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          {report.icon && (
                            <img src={report.icon} alt={report.cafeName} className="w-8 h-8 rounded" />
                          )}
                          <span className="text-sm text-slate-700">{report.cafeName}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
