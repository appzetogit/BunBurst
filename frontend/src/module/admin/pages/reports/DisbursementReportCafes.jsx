...
import { disbursementReportCafesDummy, disbursementStats } from "../../data/disbursementReportCafesDummy"
...
export default function DisbursementReportCafes() {
  const [searchQuery, setSearchQuery] = useState("")
  const [disbursements, setDisbursements] = useState(disbursementReportCafesDummy)
  ...
  const filteredDisbursements = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(disbursement =>
        ...
        disbursement.cafeName.toLowerCase().includes(query)
      )
    }
    ...
    if (filters.cafe !== "All cafes") {
      result = result.filter(d => d.cafeName === filters.cafe)
    }
  ...
  const handleExport = (format) => {
    ...
    switch (format) {
      case "csv": exportReportsToCSV(filteredDisbursements, headers, "disbursement_report_cafes"); break
      case "excel": exportReportsToExcel(filteredDisbursements, headers, "disbursement_report_cafes"); break
      case "pdf": exportReportsToPDF(filteredDisbursements, headers, "disbursement_report_cafes", "Cafe Disbursement Report"); break
      case "json": exportReportsToJSON(filteredDisbursements, "disbursement_report_cafes"); break
    }
  }
  ...
  const activeFiltersCount = (filters.zone !== "All Zones" ? 1 : 0) + (filters.cafe !== "All cafes" ? 1 : 0) + (filters.paymentMethod !== "All Payment Method" ? 1 : 0) + (filters.status !== "All status" ? 1 : 0) + (filters.time !== "All Time" ? 1 : 0)
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl font-bold text-slate-900">Cafe Disbursement Report</h1>
          </div>
        </div>
                ...
                {filteredDisbursements.length === 0 ? (
                  ...
                  filteredDisbursements.map((disbursement) => (
                    <tr key={disbursement.sl} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-slate-700">{disbursement.cafeName}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
        ...
        <DialogContent className="max-w-md bg-white p-0 opacity-0 data-[state=open]:opacity-100 data-[state=closed]:opacity-0 transition-opacity duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=open]:fade-in-0 data-[state=closed]:fade-out-0 data-[state=open]:scale-100 data-[state=closed]:scale-100">
          ...
          <div className="px-6 pb-6">
            <p className="text-sm text-slate-700">
              Cafe disbursement report settings and preferences will be available here.
            </p>
          </div>
