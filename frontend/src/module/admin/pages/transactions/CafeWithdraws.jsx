...
export default function CafeWithdraws() {
  ...
  const filteredWithdraws = useMemo(() => {
    ...
    if (searchQuery.trim()) {
      ...
      result = result.filter(w =>
        w.cafeName?.toLowerCase().includes(query) ||
        w.cafeIdString?.toLowerCase().includes(query) ||
  ...
  const handleExport = async (format) => {
    ...
    const headers = [
      ...
      { key: "cafeIdString", label: "Cafe ID" },
    ...
    const exportData = filteredWithdraws.map((w, index) => ({
      ...
      cafeName: w.cafeName || 'N/A',
      cafeIdString: w.cafeIdString || 'N/A',
    ...
    switch (format) {
      case "excel":
        exportTransactionsToExcel(exportData, headers, "cafe_withdraws_full_details")
        break
      case "pdf":
        await exportTransactionsToPDF(exportData, headers, "cafe_withdraws_full_details", "Cafe Withdraws Report")
        break
      default: break
    }
  }
              ...
              <table className="w-full">
                <thead className="bg-slate-50 border-b border-slate-200">
                  <tr>
                    ...
                    {visibleColumns.cafe && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">Cafe Name</th>}
                    {visibleColumns.cafeId && <th className="px-6 py-4 text-left text-[10px] font-bold text-slate-700 uppercase tracking-wider">Cafe ID</th>}
                  ...
                  {filteredWithdraws.length === 0 ? (
                    ...
                    filteredWithdraws.map((withdraw, index) => (
                      <tr key={withdraw.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        {visibleColumns.cafe && <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-slate-700">{withdraw.cafeName || 'N/A'}</span>
                        </td>}
                        {visibleColumns.cafeId && <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm font-medium text-slate-700">{withdraw.cafeIdString || 'N/A'}</span>
                        </td>}
            ...
            {selectedWithdraw && (
              <div className="px-6 pb-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-slate-900 mt-1">{selectedWithdraw.cafeName || 'N/A'}</p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-slate-500 uppercase">Cafe ID</label>
                  <p className="text-sm font-medium text-slate-900 mt-1">{selectedWithdraw.cafeIdString || 'N/A'}</p>
                </div>
                <div>
